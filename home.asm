...
.skipSwitchToMapBank
	...
	ld [hVBlankCounter], a ; used as joypad poll timer
	ld hl, wMapTextPtr
	ld a, [hli]
	ld h, [hl]
...
CloseTextDisplay::
	...
	call LoadGBPal
	xor a
	ld [hBGMapMode], a ; disable continuous WRAM to VRAM transfer each V-blank
...
DisplayListMenuID::
	xor a
	ld [hBGMapMode], a ; disable auto-transfer
	ld a, 1
...
DisplayListMenuIDLoop::
	xor a
	ld [hBGMapMode], a ; disable transfer
	call PrintListMenuEntries
	ld a, 1
	ld [hBGMapMode], a ; enable transfer
	call Delay3
	ld a, [wBattleType]
	and a ; is it the Old Man battle?
...
.Machine
	call GetMachineName
.Finish
	ld de, wStringBuffer ; pointer to where item name is stored in RAM
	pop bc
	pop hl
	ret

GetMachineName::
; copies the name of the TM/HM in [wd11e] to wStringBuffer
	push hl
	push de
	push bc
	ld a, [wd11e]
...
GetMoveName::
	...
	ld [wPredefBank], a
	call GetName
	ld de, wStringBuffer ; pointer to where move name is stored in RAM
	pop hl
	ret
...
GetSpritePosition1:: ; Return to HRAM
	ld hl, _GetSpritePosition1
	jr SpritePositionBankswitch

GetSpritePosition2:: ; Return to WRAM
	ld hl, _GetSpritePosition2
	jr SpritePositionBankswitch

SetSpritePosition1:: ; Read from HRAM
...
; also calls .UnnecessaryCall, which is a bankswitch to IsStarterPikachuInOurParty
	ld a, [wd472]
	and $3f
	ld [wd472], a
	ld hl, wPartyMon1
...
; INPUT:
; hl = address of array
; [hSpriteIndexOrTextID] = index of boulder sprite
; OUTPUT:
; [wCoordIndex] = if there is match, the matching array index
; sets carry if the coordinates are in the array, clears carry if not
CheckBoulderCoords::
...
; sets movement byte 1 for sprite [hSpriteIndexOrTextID] to $FE and byte 2 to [hSpriteMovementByte2]
SetSpriteWalkingWithinGivenRange::
	push hl
	call GetSpriteMovementByte1Pointer
	ld [hl], WALK
	call GetSpriteMovementByte2Pointer
	ld a, [hSpriteMovementByte2]
	ld [hl], a ; walk in range
	pop hl
	ret

; sets both movement bytes for sprite [hSpriteIndexOrTextID] to $FF
StopSprite::
	push hl
	call GetSpriteMovementByte1Pointer
	ld [hl], STAY
...
; returns the sprite movement byte 1 pointer for sprite [hSpriteIndexOrTextID] in hl
...
; returns the sprite movement byte 2 pointer for sprite [hSpriteIndexOrTextID] in hl
...
MoveSprite::
; move the sprite [hSpriteIndexOrTextID] with the movement pointed to by de
...
; loads screen tiles stored in wTileMapBackup2 but leaves hBGMapMode disabled
...
; The reason is that most functions that call this initialize hDownArrowBlinkCnt1 to 0.

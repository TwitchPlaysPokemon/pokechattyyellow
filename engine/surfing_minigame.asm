...
.loop
	...
	call SurfingPikachu_UpdateObjectXCoord
	call .DelayFrame
...
.Jumptable:
	...
	dw SurfingMinigame_SpeedTowardsEnd ; 2
	dw SurfingMinigame_SpeedTowardsEnd2 ; 3
...
SurfingMinigameRoutine_SpawnPikachu:
	...
	inc [hl]
	ld a, $1
	ld [wSurfingMinigameUpdateMusicTempo], a
	ret
...
SurfingMinigame_RunGame:
	...
	call SurfingMinigame_UpdateWave
	call Func_f844c
...
.dead
	ld a, $1
	ld [wSurfingMinigameFailed], a
	ld a, $c
	ld [wSurfingMinigameRoutineNumber], a
	ld a, 128
	ld [wSurfingMinigameFailureScreenTimer], a
	ld a, $b
	lb de, $88, $58
	call SpawnAnimatedObject
...
SurfingMinigame_SpeedTowardsEnd:
	...
	call SurfingMinigame_UpdatePikachuYCoord
	call SurfingMinigame_UpdateBGMapReadBuffer
	call SurfingMinigame_UpdateWaveFast
...
.done_delay
	...
	ld a, $4
	ld [wSurfingMinigamePikachuObjectFunctionIndex], a
	xor a
	ld [hLCDCPointer], a
...
SurfingMinigame_SpeedTowardsEnd2:
	ld a, [hSCX]
	and a
	jr z, .done
	call SurfingMinigame_UpdateLYOverrides
	...
	call SurfingMinigame_UpdateWaveContinue
	ret
...
SurfingMinigame_GameOver:
	...
	call SurfingMinigame_UpdateWave
	call SurfingMinigame_ResetMusicTempo
	ld hl, wSurfingMinigameFailureScreenTimer
	ld a, [hl]
	and a
	jr z, .wait_press_a
	dec [hl]
...
SurfingMinigameAnimatedObjectFn_Pikachu
	ld a, [wSurfingMinigamePikachuObjectFunctionIndex]
	ld e, a
	ld d, $0
...
SurfingPikachu_UpdateObjectXCoord:
	ld a, [wc635]
	ld e, a
	ld d, $0
	ld a, [wc5e3]
...
SurfingMinigame_UpdateBGMapReadBuffer:
	ld a, [wSurfingMinigameBGMapReadBuffer] ; ???

	; hl = vBGMap0 + ([hSCX] + $48) / 8
	; c = [wSurfingMinigamePikachuObjectHeight] / 8
	ld a, [hSCX]
	add $48
	ld e, a
	srl e
...
SurfingMinigame_UpdatePikachuYCoord:
	ld a, [hSCX]
	and $8
	jr nz, .use_tile_8
...
SurfingMinigame_UpdateWaveFast:
	ld a, $a0
	ld [wSurfingMinigameXOffset], a
	ld a, [hSCX]
	ld h, a
	...
	ld a, h
	ld [hSCX], a
	jr SurfingMinigame_UpdateWaveContinue

SurfingMinigame_UpdateWave:
	ld a, $a0
	ld [wSurfingMinigameXOffset], a
	ld a, [hSCX]
	ld h, a
	...
	ld a, h
	ld [hSCX], a
SurfingMinigame_UpdateWaveContinue:

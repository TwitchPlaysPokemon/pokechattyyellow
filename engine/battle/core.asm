...
; keep in mind that the experience value has already been divided by the size of the party
	ld a, $1
	ld [wBoostExpByExpAll], a
	ld a, [wPartyCount]
	ld b, 0
...
; bug: when this is called, [hBGMapMode] is non-zero, so there is screen tearing
...
.doEnemyMonAnimation
	ld a, BANK(AnimationSubstitute) ; BANK(AnimationMinimizeMon)
	rst FarCall
...
ExecutePlayerMove:
	xor a
	ld [hBattleTurn], a ; set player's turn
	ld a, [wPlayerSelectedMove]
	inc a
...
playPlayerMoveAnimation:
	...
	condfarcall nz, HideSubstituteShowMonAnim
	pop af
	ld [wAnimationType], a
	ld a, [wPlayerMoveNum]
	call PlayMoveAnimation
	...
	ld a, [wPlayerBattleStatus2]
	bit HasSubstituteUp, a
	condfarcall nz, ReshowSubstituteAnim
	jr MirrorMoveCheck
...
.skip
	...
	ld a, [wEnemyBattleStatus2]
	bit HasSubstituteUp, a ; does mon have a substitute?
	jr z, EnemyCheckIfMirrorMoveEffect
	callab ReshowSubstituteAnim ; slide the substitute's sprite out
...
.inflictPoison
	...
	ld a, [hBattleTurn]
	and a
	ld b, PLAYER_WAS_POISONED_ANIM
	ld hl, wPlayerBattleStatus3
	ld a, [de]
	ld de, wPlayerToxicCounter
	jr nz, .ok
	ld b, ENEMY_WAS_POISONED_ANIM
	ld hl, wEnemyBattleStatus3
	ld de, wEnemyToxicCounter
.ok
	cp TOXIC
...
.statModifierUpEffect
	...
	cp ATTACK_DOWN1_EFFECT - ATTACK_UP1_EFFECT ; covers all +1 effects
...
.incrementStatMod
	...
	ld a, [de]
	cp ATTACK_DOWN1_EFFECT ; is it a +2 effect?
	jr c, .ok
...
.ok
	ld [hl], b
	ld a, c
	cp ACCURACY_UP1_EFFECT - ATTACK_UP1_EFFECT
